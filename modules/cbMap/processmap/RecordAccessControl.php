...
class RecordAccessControl extends processcbMap {
	...
	private function convertMap2Array() {
		...
		if (isset($xml->listview->c)) $this->mapping['listview']['c'] = (Integer)$xml->listview->c;
		if (isset($xml->listview->r)) $this->mapping['listview']['r'] = (Integer)$xml->listview->r;
		if (isset($xml->listview->u)) $this->mapping['listview']['u'] = (Integer)$xml->listview->u;
		if (isset($xml->listview->d)) $this->mapping['listview']['d'] = (Integer)$xml->listview->d;
		...
		if (isset($xml->detailview->c)) $this->mapping['detailview']['c'] = (Integer)$xml->detailview->c;
		if (isset($xml->detailview->r)) $this->mapping['detailview']['r'] = (Integer)$xml->detailview->r;
		if (isset($xml->detailview->u)) $this->mapping['detailview']['u'] = (Integer)$xml->detailview->u;
		if (isset($xml->detailview->d)) $this->mapping['detailview']['d'] = (Integer)$xml->detailview->d;
		...
		if (isset($xml->relatedlists)) {
			foreach($xml->relatedlists->relatedlist as $k=>$v) {
				...
				if (isset($v->c)) $this->mapping['relatedlist'][$modulename]['c'] = (Integer)$v->c;
				if (isset($v->r)) $this->mapping['relatedlist'][$modulename]['r'] = (Integer)$v->r;
				if (isset($v->u)) $this->mapping['relatedlist'][$modulename]['u'] = (Integer)$v->u;
				if (isset($v->d)) $this->mapping['relatedlist'][$modulename]['d'] = (Integer)$v->d;
				if (isset($v->s)) $this->mapping['relatedlist'][$modulename]['s'] = (Integer)$v->s;
	...
	private function convertConditionMap2Array($condition) {
		...
		if (!empty($condition->businessrule)) {
			...
			if (isset($condition->c)) $cmap['c'] = (Integer)$condition->c;
			if (isset($condition->r)) $cmap['r'] = (Integer)$condition->r;
			if (isset($condition->u)) $cmap['u'] = (Integer)$condition->u;
			if (isset($condition->d)) $cmap['d'] = (Integer)$condition->d;
			if (isset($condition->s)) $cmap['s'] = (Integer)$condition->s;
	...
	*/
	public function hasRelatedListPermissionTo($operation,$onmodule,$default=true) {
		if (empty($onmodule)) return $default;
		if (count($this->mapping)==0) $this->convertMap2Array();
